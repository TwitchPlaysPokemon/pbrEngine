...
class Monitor(object):
    ...
    def reprint(self):
        ...
        for i in range(max(lenBlue, lenRed)):
            ...
            print(" | %s  %-18s|%18s  %s |" % (
                ("X" if not self.pbr.match.aliveBlue[i] else (">" if i == self.pbr.match.currentBlue else " ")) if blue else " ",
                blue["name"] if blue else " ",
                red["name"] if red else " ",
                ("X" if not self.pbr.match.aliveRed[i] else ("<" if i == self.pbr.match.currentRed else " ")) if red else " ",
            ))
        print(" +----------------------+----------------------+")
        ...
        for i in range(self.max_events):
            try:
                print (" | %-43s |" % (self.events[i][:41]+".." if len(self.events[i]) > 43 else self.events[i]))
            except:
                print(" |                                             |")
        print(" +---------------------------------------------+")
