...
class DolphinIO:
    ...
    def readMulti(self, readTuples, numAttempts=3):
    ...
    def write(self, mode, addr, val, **kwargs):
        return self.writeMulti([(mode, addr)], **kwargs)[0]

    def writeMulti(self, writeTuples, maxAttempts=3, writesPerAttempt=3,
        ...
        '''
        writes_needed = list(writeTuples)
        ...
        assert (maxAttempts == 1 if readsPerAttempt == 0 else True,
                "maxAttempts must be 1 when not verifying reads (readsPerAttempt == 0)")
    ...
    def readNestedAddr(self, nestedLocation, maxAttempts=5, readsPerAttempt=3,
        ...
        for offset in nestedLoc.offsets:
            val = 0
            for i in range(maxAttempts):
                val = self.read32(loc, num_reads=readsPerAttempt)
                if isValidLoc(val):
                    break
                else:
